...
音乐创作页提供“一键参考创作”，复用既有线性工作流，不增加第二套音乐队列或结果表：

```text
GET  /api/v1/music/reference-songs?query=歌名或歌手
POST /api/v1/music/reference-runs
  -> analysis（指定 RankingEntry）
  -> lyrics（报告首选方向）
  -> music（Redis 队列）
```

- 搜索范围是保留期内的 `ranking_entries`，同一 `source_song_id` 只显示最新采集版本。
- 请求只需要 `source_entry_id` 和可空的 `instruction`。空输入使用默认完整歌曲规则；
  非空输入同时进入作词与音乐要求。
- 运行记录使用 `template_id = null` 的 `WorkflowRun`，步骤仍保存真实 `task_id` 和
  `output_id`，因此刷新页面后可以恢复进度，也能继续使用工作流详情抽屉查看产出。
- 启动账号必须同时具有分析、作词和音乐权限；音乐步骤创建时按普通生成任务扣除一次
  员工额度，文本和音乐接口继续分别进入统一用量账本。
- 当前参考分析基于榜单元数据和趋势证据，不读取来源歌曲音频。外部曲库搜索、授权音频
  上传或官方 Cover/Remix 以后通过独立 Provider 能力扩展。
...
当前基线：后端 `88` 个测试，前端 `28` 个测试。兼容适配器测试覆盖生成归一化、
