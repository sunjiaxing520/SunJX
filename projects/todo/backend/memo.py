...
async def read_today(key):
    async def request(client, method, payload):
        ...
        if response.status_code != 200:
            raise HTTPException(502,{401:'墨墨 API Key 无效，请重新绑定。',403:'墨墨未授权读取学习数据，请检查开放 API 权限。',429:'墨墨查询频率受限，请稍后刷新。'}.get(response.status_code,'墨墨暂时不可用，请稍后重试。'))
        body=response.json()
        # The study service uses direct objects, unlike older content APIs.
    ...
    except httpx.TimeoutException: raise HTTPException(504,'墨墨同步超时，请稍后刷新。')
    except httpx.RequestError: raise HTTPException(502,'无法连接墨墨，请稍后刷新。')
    except (ValueError,KeyError,TypeError,ValidationError): raise HTTPException(502,'墨墨返回的数据格式不完整，请先打开 App 并开启自动同步后重试。')
