...
(async () => {
  ...
  await phone.getByRole('combobox',{name:'优先级',exact:true}).click();
  await phone.getByRole('option',{name:'优先完成',exact:true}).click();
  await phone.getByRole('combobox',{name:'掌握情况',exact:true}).click();
