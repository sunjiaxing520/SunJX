...
(async () => {
  ...
  await page.getByRole('button',{name:'创建账号',exact:true}).click();
  await expect(page.getByText(/先看看一份示例计划/)).toHaveCount(0);
  await page.getByLabel('添加待办内容').fill('看懂 HTML 的基本结构');
  ...
  await browser.close();
  console.log('PASS: registration, no-key Todo, manual task, two-context sync, mobile edit, 4 widths × 5 views, mobile AI, no page errors.');
