...
主线：固定工作流，内部按独立 Agent 模块实现
阶段：P0-P6 已完成；P7 音乐生成待选型与开发；P8 联调交付待开始
文本 AI：智谱 glm-4.7-flash，仅用于开发联调，可热切换
运行：Docker 跑 PostgreSQL/Redis，本地进程跑 FastAPI/Vite
测试基线：后端 45；前端 9（以当前实际 pytest/vitest 输出为准）
首要下一步：确定正式音乐生成 API，进入 P7
```
...
| `N04` | 首期工作流 | 登录 -> 酷狗采集 -> 榜单分析 -> AI 作词 -> 官方音乐生成 API -> 结果展示 | `分阶段开发任务书.md` |
| `N05` | 范围 | 首期固定流程；不做自定义工作链、总调度 AI、多作词 Agent、复杂长期记忆、自研音乐模型 | `需求规格说明书_v2.md`、决策记录 |
...
| `N10` | P4 榜单 | 酷狗 TOP500、自然日快照、同日更新、30 天保留、最近 15 次运行、详情折叠展示 | 决策记录、P4-P6 维护说明 |
| `N11` | P5 分析 | 连续榜单趋势、结构化创作方向；只依据元数据，不伪装音频分析；速度统一为 slow/medium/fast | P4-P6 维护说明 |
| `N12` | P6 作词 | 生成、重新生成、版本保存、标准段落和统一音乐创作方案已完成 | P4-P6 维护说明 |
...
| `N15` | 收藏夹 | 团队共享；收藏具体分析报告或歌词版本；支持分类、时间、创建人和隐藏备注 | 决策记录 |
| `N16` | 用量看板 | `api_usage_records` 统一记录接口、模型、Token、耗时、尝试次数和供应商请求编号 | 决策记录 |
| `N17` | P7 音乐 | 尚未实现；候选为 Mureka、MiniMax 等正式官方 API；不得接非官方 Suno 套壳 | 决策记录 |
| `N18` | 音乐结果 UX | 生成结果进入独立试听区；试听不额外生成；每首独立播放/下载；支持“首段 -> 试听 -> 主动续写” | 决策记录 |
| `N19` | 执行模型 | 当前分析和作词在 FastAPI 进程内同步执行；超时任务可自动收尾；真正后台队列属于后续工程化 | 错误手册、代码 |
| `N20` | 数据保留 | 普通任务产物计划保留 30 天；收藏产物应跳过清理；重复采集保留当天最新快照 | 决策记录 |
...
- 不为“看起来成功”而吞掉外部错误。
- 不在 P7 选型前写死音乐供应商。
- 不为有限预算提前实现总调度 AI、复杂记忆或多 Agent 市场。
