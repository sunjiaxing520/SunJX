...
测试基线：后端 81；前端 15（以当前实际 pytest/vitest 输出为准）
...
| `N12` | P6 作词 | 生成、重新生成、版本保存、标准段落、统一音乐创作方案、任务单删和批量删除已完成；引用方向含最新两份和收藏分析，AI 助手预览只在确认后生成新版本 | P4-P6 维护说明、P8 维护说明 |
...
| `N29` | 审核智能体 | 独立审核页；管理员经初始化对话创建人设、规则和长期记忆并分配成员；成员只见记忆概述，完整记忆仅管理员展开；记忆显式保存并由 AI 去重，首期审核歌词 | P8 维护说明 |
