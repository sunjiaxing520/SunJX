...
阶段：P0-P6.5 已完成；P7 Suno 平台基础已完成，真实官方 API 契约待联调；P8 联调交付待开始
文本 AI：智谱 glm-4.7-flash，仅用于开发联调，可热切换
运行：Docker 跑 PostgreSQL/Redis，本地进程跑 FastAPI/Vite
测试基线：后端 61；前端 15（以当前实际 pytest/vitest 输出为准）
首要下一步：登录 Suno Platform 获取正式文档、API Key 和商用权限，完成 `SunoOfficialMusicProvider` 的真实 HTTP 契约
```
...
| `N17` | P7 音乐 | 首期锁定 Suno；音乐任务、结果、试听、下载、续写、删除、用量和四步工作流基础已完成；真实 HTTP 契约仍待 Platform 文档、Key 和商用权限；不得接非官方套壳或网页自动化 | 决策记录、P7 维护说明 |
| `N18` | 音乐结果 UX | 生成结果进入独立试听区；试听不额外生成；每首独立播放/下载；支持“首段 -> 试听 -> 主动续写” | 决策记录 |
| `N19` | 执行模型 | 手工分析和作词仍同步；分析页发现活动任务后每 2.5 秒轮询且后端禁止并发分析；自动流程用 FastAPI 后台任务立即返回并轮询；进程重启会中断，Redis Worker 属于后续工程化 | 错误手册、P6.5 维护说明、决策记录 |
