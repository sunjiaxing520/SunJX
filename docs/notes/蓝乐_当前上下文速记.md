...
测试基线：后端 70；前端 15（以当前实际 pytest/vitest 输出为准）
首要下一步：登录 Suno Platform 获取正式文档、API Key 和商用权限，完成 `SunoOfficialMusicProvider` 合同；演示期如启用兼容实现，只连接内网隔离服务并人工处理 hCaptcha
```
...
| `N17` | P7 音乐 | 业务层统一使用 `suno` Provider；`official` 为默认且真实合同等待 Platform 账号内文档；`compatibility` 已适配 gcui-art/suno-api 的生成/续写/查询/额度合同，默认关闭且只允许隔离内网；不集成 2Captcha、指纹规避或验证码绕过 | P7 维护说明 |
